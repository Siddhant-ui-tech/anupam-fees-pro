...
import { Plus, Search, Pencil, Trash2, Eye, Check } from "lucide-react";
...
export default function Students() {
  ...
  const handlePayFee = async (s: Student) => {
    ...
    await supabase.from("students").update({ next_due_date: nextDue }).eq("id", s.id);
                ...
                <CardContent className="p-3">
                  ...
                  <div className="flex items-center gap-1.5 mt-2">
                    {status !== "paid" && (
                      ...
                      >
                        <Check className="h-3 w-3 mr-1" /> Fee Received
                      </Button>
                    )}
                    ...
                    <Button size="sm" variant="outline" className="h-8 px-2 text-destructive" onClick={() => handleDelete(s.id)}>
        ...
        <DialogContent className="max-w-sm mx-auto">
          ...
          <div className="space-y-3">
            <div>
              ...
              <Input value={name} onChange={e => setName(e.target.value)} className="h-11" placeholder="Student name" />
            </div>
            <div>
              <Label className="text-sm">Phone</Label>
              <Input value={phone} onChange={e => setPhone(e.target.value)} className="h-11" placeholder="Phone number" type="tel" />
            </div>
            <div>
            ...
            <div>
              ...
              <Input type="date" value={joiningDate} onChange={e => setJoiningDate(e.target.value)} className="h-11" />
            </div>
            <div>
              <Label className="text-sm">Monthly Fee (₹)</Label>
              <Input type="number" value={monthlyFee} onChange={e => setMonthlyFee(e.target.value)} className="h-11" placeholder="1000" />
            </div>
